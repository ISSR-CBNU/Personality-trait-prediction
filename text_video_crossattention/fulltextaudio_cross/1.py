...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, video, audio):
        ...
        B,_,_,_,C = video.shape
        video = video.view(B,-1,C)
        # LayerNorm after dropout
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.audio_representations = AudioRepresentations(audio_split_samples, hid_dim, audio_representation_layers,
                                                          n_heads, pf_dim, audiodrop, max_length)
        self.pos_drop = nn.Dropout(p=drop_rate)

        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
        self.cross_attention = nn.ModuleList(
            [CrossAttentionLayer(hid_dim, n_heads, pf_dim, audiodrop) for _ in range(audio_representation_layers)])
#         self.cross_attention = nn.ModuleList(
#             [CrossAttentionLayer(hid_dim, n_heads, pf_dim, dropout) for _ in range(cross_attention_layers)])
        ...
        self.videodim = nn.Linear(int(embed_dim * 2**self.num_layers),hid_dim)
        self.custom_layer_1 = nn.Linear(24*hid_dim, custom_layer_1outdim)
    ...
    def forward(self, video,audio):
        ...
        for layer in self.layers:
            video = layer(video.contiguous())
        # video = rearrange(video, 'b c d h w -> b d h w c')
        # print(video.shape)
        # video = self.videodim(video)
...
#         x = rearrange(x, 'B N C -> B C N')
        B,N,C = x.shape
#         n = int(round(N ** (1/3)))
#         x = rearrange(x, 'B C (n1 n2 n3) -> B C n1 n2 n3', n1=n, n2=n, n3=n)
