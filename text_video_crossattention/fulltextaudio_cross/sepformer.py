...
class Encoder(nn.Module):
    ...
    torch.Size([2, 64, 499])
    """
    def __init__(self, kernel_size=16, out_channels=256, in_channels=24):
...
class Cross_Dual_Path_Model(nn.Module):
    ...
    def forward(self, x, video):
        ...
        # v = rearrange(v, 'b d c h w -> b c d h w')
        # video = self.r2plus1d_18(video)
        #torch.Size([8, 512, 2, 14, 14])
...
class Cross_Sepformer(nn.Module):
    def __init__(self,
        ...
        self.Separator = Cross_Dual_Path_Model(num_spks=num_spks,
                                               ...
                                               skip_around_intra=True)
        self.num_spks = num_spks
        audio_chunk = ((audio_f-16)//8)+1
        audio_f = ((audio_chunk-1)*8)+16
        self.layer1 = nn.Linear(audio_f,768)
    ...
    def forward(self, mix, video):
        ...
        # print("separator",est_mask.shape)
        ...
        est_source = est_source.squeeze()
        est_source = self.layer1(est_source)
        # est_source = self.layer2(est_source)
        # est_source = self.layer3(est_source)
