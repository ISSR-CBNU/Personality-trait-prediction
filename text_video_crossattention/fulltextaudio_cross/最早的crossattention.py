...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
                 ...
                 patch_size=(4,4,4),
                 embed_dim=96,
                 depths=[2, 2, 6, 2],#[2,2,18,2]->[2,2,6,2]
                 num_heads=[3,6,12,24],
                 window_size=(2,7,7),
                 mlp_ratio=4.,
                 qkv_bias=True,
...
#         self.custom_layer_1 = nn.Linear(int(embed_dim * 2**fusionLayer), custom_layer_1outdim)
    ...
    def forward(self, video,lip,text,audio):
        """Forward function."""
        ...
        #     lip = layers(lip)
        # lip = rearrange(lip,"B C D H W  -> B (D H W) C")
        ...
        for layers in self.layers:
            if i == 3:
                ...
                for layer in  self.cross_attention:
                    # if crossattention_layer_count%2 == 0:
                    #     x = layer(x.contiguous(),lip.contiguous())
                    # elif crossattention_layer_count%3==0:
