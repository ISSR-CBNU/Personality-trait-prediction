...
class MultiHeadedAttentionComponent(nn.Module):
    ...
    def __init__(self, hid_dim, n_heads, dropout,dim,num_layers,audio_dim):
        ...
        self.fc_q = nn.Linear(dim*2**(num_layers-1), hid_dim)
        # Transformation from source vector to key vector
        self.fc_k = nn.Linear(audio_dim, hid_dim)

        # Transformation from source vector to value vector
        self.fc_v = nn.Linear(audio_dim, hid_dim)
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, video, audio):
        ...
        _video = rearrange(_video, 'B (T H W) C -> B T H W C', T=T,H=H,W=W)
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
                 ...
                 audio_lenth = 24,audio_dim = 384,#768->384
                 n_heads = 8,
                 fusionLayer=3,
                 hid_dim = 256,
                 fusion_dim = 384,
        ...
        self.con1d=nn.Conv1d(in_channels=768,out_channels=384,kernel_size=1)
        ...
        self.custom_layer_1 = nn.Linear(self.num_features, custom_layer_1outdim)
#         self.custom_layer_1 = nn.Linear(int(embed_dim * 2**fusionLayer), custom_layer_1outdim)
    ...
    def forward(self, video,audio,lip):
        """Forward function."""
        ...
        lip = lip.permute(0,4,1,2,3)
        lip = self.r2plus1d_18(lip)
        lip = self.patch_embed3d(lip)
        lip = self.pos_drop(lip)
        for layers in self.liplayers:
            lip = layers(lip)
        lip = rearrange(lip,"B C D H W  -> B C (D H W)")
        lip = self.con1d(lip)    
        lip = rearrange(lip,"B C O  -> B O C")
        # print("lip",lip.shape)
        i=1
        crossattention_layer_count = 1
        # audio = self.AV_sepformer(audio,video4)
        for layers in self.layers:
            if i == 3:
                video = rearrange(video, 'b c d h w -> b d h w c')
                for layer in  self.cross_attention:
                    if crossattention_layer_count < 10:
                        video = layer(video.contiguous(),lip.contiguous())
                    else:
                        video = layer(video.contiguous(),audio.contiguous())
                    crossattention_layer_count+=1
                video=self.patch_merging(video)
                video = rearrange(video,'b d h w c -> b c d h w')
            else:
