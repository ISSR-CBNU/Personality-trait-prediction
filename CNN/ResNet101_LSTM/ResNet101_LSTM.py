...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv3d(15, self.in_channel, kernel_size=7, stride=2,
...
class ResNetLSTM(nn.Module):
    def __init__(self, resnet_out_size, lstm_input_size, lstm_hidden_size, lstm_num_layers, lstm_output_size):
        super(ResNetLSTM, self).__init__()
        self.resnet_encoder = ResNet(Bottleneck, [3, 4, 23, 3], num_classes=resnet_out_size, include_top=True)
        self.lstm_model = LSTMModel(input_size=lstm_input_size, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers, output_size=lstm_output_size)

    def forward(self, x):
        resnet_out = self.resnet_encoder(x)
        lstm_in = resnet_out.unsqueeze(1)
        lstm_out = self.lstm_model(lstm_in)
        return lstm_out