...
class InceptionV2(nn.Module):
    def __init__(self, num_classes=1000,batch_size=5 ,stage='train'):
        ...
        self.linear = nn.Linear(15*1024, 512)
    ...
    def forward(self, x):
        ...
        out = self.linear(x)
        return out
class InceptionV2_0(nn.Module):
    def __init__(self, num_classes=1000,batch_size=5 ,stage='train'):
        super(InceptionV2_0, self).__init__()
        self.stage = stage
        self.batch_size=batch_size
        self.block1 = nn.Sequential(
            ConvBNReLU(in_channels=3, out_channels=64, kernel_size=7,stride=2,padding=3),
...
class LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc(out[:, -1, :])
        return out
class inceptionv2_LSTM(nn.Module):
    def __init__(self, num_classes=5,batch_size=5,hidden_size=256,num_layers=2):
        super(inceptionv2_LSTM, self).__init__()
        self.inception = InceptionV2(num_classes,batch_size)
        self.lstm = LSTM(input_size=512, hidden_size=hidden_size, num_layers=num_layers, num_classes=num_classes)
