...
class Model2(nn.Module):
    def __init__(self, image_size, patch_size, num_classes, num_frames, dim = 192, depth = 4, heads = 3, pool = 'cls', in_channels = 512, dim_head = 64, dropout = 0.,
...
class R2Plus1D_ViViT(nn.Module):
    ...
    def __init__(self, t, h, w, patch_t, patch_h, patch_w, num_classes, dim, depth, heads, mlp_dim, dim_head=3,
        ...
        self.dropout = nn.Dropout(emb_dropout)
        if model ==2:
            self.transformer = Model2(h, patch_h, num_classes, t, dim,depth,heads)
        elif model == 3:
